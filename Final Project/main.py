...
class Game:
    ...
    def load_data(self):
        ...
        for i in range(1,4):
            ...
            '''
            (oof new clouds don't work)
            self.cloud_images.append(pg.image.load(path.join(img_dir, 'storm cloud{}.png'.format(i))).convert())
            '''
        # load sounds
        # Use 'Better Jump' because it is so much better
        self.snd_dir = path.join(self.dir, 'snd')
        self.jump_sound = [pg.mixer.Sound(path.join(self.snd_dir, 'Better Jump.wav')),
                            pg.mixer.Sound(path.join(self.snd_dir, 'Better Jump.wav'))]
        self.boost_sound = pg.mixer.Sound(path.join(self.snd_dir, 'Better Jump.wav'))
        self.head_jump_sound = pg.mixer.Sound(path.join(self.snd_dir, 'Better Jump.wav'))
    def new(self):
        self.score = 0
        ...
        for i in range(8):
            c = Cloud(self)
            c.rect.y += 500
        # load music that doesn't annoy me to no end
        pg.mixer.music.load(path.join(self.snd_dir, '09 Level 5.mp3'))
        # call the run method
        self.run()
    def run(self):
        # game loop
    ...
    def update(self):
        ...
        # if now - self.mob_timer > 5000 + random.choice([-1000, -500, 0, 500, 1000], "Flie_Balrog.png"):
            self.mob_timer = now
            Mob(self)
    ...
    def draw(self):
        ...
        '''
        (background image doesn't work so I guess I'll just use red)
        background_image = pg.image.load("backgroun2.gif").convert()
